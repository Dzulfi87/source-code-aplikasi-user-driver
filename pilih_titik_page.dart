...
import 'package:driver_angkutan_sayur/presentation/statemanagement/bloc/pilih_titik/pilih_titik_bloc.dart';
import 'package:driver_angkutan_sayur/presentation/statemanagement/bloc/pilih_titik/pilih_titik_state.dart';
...
class PilihTitikPage extends StatelessWidget {
  const PilihTitikPage({Key? key, required this.isAsal, required this.currentLocationBloc}) : super(key: key);
  final bool isAsal;
              ...
              builder: (BuildContext context, state) {
                return Row(
                  children: [
                    ...
                    Visibility(
                        visible: state is SearchState || state is SearchingState || state is SearchDoneState,
                        child: TextButton(
                          onPressed: () {
          ...
          BlocBuilder<PilihTitikBloc, PilihTitikState>(
            builder: (BuildContext context, state) {
              ...
              if(state is SearchDoneState){
                ...
                if(state.moveMarker) {
                  _controller.animateCamera(CameraUpdate.newLatLngZoom(state.marker.position,20));
                }
              }
              return Expanded(
                child: Stack(
                    ...
                    GoogleMap(
                      ...
                      onCameraIdle: (){
                        if(state is! InitialState) {
                          _controller.getVisibleRegion().then((value) => context.read<PilihTitikBloc>().onMapsMove(value));
                        }
                      },
                    ),
                    Center(
                          ...
                          children: [
                            Card(
                              ...
                              child: Padding(
                                ...
                                child: Text(state is MarkerLoadedState ? _markers.infoWindow.title! : state is LoadingLatLngState ? "Loading..." : ""),
                              ),
                            ),
                            const Icon(
                              Icons.location_pin,
                          ...
                          child: Expanded(
                            child: ListView.separated(
                              ...
                              itemBuilder: (BuildContext context, int index) {
                                ...
                                return ListTile(
                                  onTap: ()=> context.read<PilihTitikBloc>().onLokasiListTap(lokasi),
                                  title: Text(lokasi.$2),
                                );
          ...
          child: BlocBuilder<PilihTitikBloc,PilihTitikState>(
            builder: (context,state){
              return ElevatedButton(
                onPressed: state is! InitialState ? () => context.read<PilihTitikBloc>().onPilihTitikButton(_markers, context) : null,
                child:  Text(state is! InitialState ? "Pilih Titik Ini": "Tunggu sebentar..") ,
              );
            },
          ),
        ),
